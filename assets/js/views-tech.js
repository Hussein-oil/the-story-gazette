...
/* ── renderTech ──────────────────────────────────── */
function renderTech(){
  leaveReader(); setActiveNav("tech"); setBack(false);
  ...
  const catTabs=['prog','comp'].map(c=>`<button class="${c===curTechCat?'on':''}" data-tcat="${c}">${c==='prog'?t('Programming'):t('Computer')}</button>`).join('');
  const lvlTabs=TECH_LEVELS.map(l=>`<button class="${l===curTechLevel?'on':''}" data-tlvl="${l}" style="${l===curTechLevel?'border-color:'+accent+';color:'+accent:''}"><span>${l}</span><span class="tl-sub">${TECH_LEVEL_DESC[curTechCat][l]}</span></button>`).join('');
  ...
  view.innerHTML=`<div class="wrap">
    <div class="sec-head" style="margin-bottom:28px">
      ...
      <h1 class="sec-title">${t('Learn with Stories')}</h1>
    ...
    <div class="tech-cats">${catTabs}</div>
    <div class="tech-levels">${lvlTabs}</div>
    <div class="tech-grid">${cards||'<p style="color:var(--faint);font-style:italic">More stories coming soon.</p>'}</div>
  </div>`;
  view.querySelectorAll('[data-tcat]').forEach(b=>b.onclick=()=>{ curTechCat=b.dataset.tcat; renderTech(); });
  view.querySelectorAll('[data-tlvl]').forEach(b=>b.onclick=()=>{ curTechLevel=b.dataset.tlvl; renderTech(); });
...
function renderTechReader(){
  ...
  view.innerHTML=`<div class="wrap-read">
    <article>
      ...
      <div class="reader-audio reveal" style="animation-delay:120ms">
        <div class="ra-main">
